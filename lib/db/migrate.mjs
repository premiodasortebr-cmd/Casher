  ...
  try {
    ...
    for (const arquivo of arquivos) {
      ...
      try {
        ...
        // A própria migration pode se registrar (quando é colada à mão no SQL Editor).
        await client.query(
          "insert into public._casher_migrations (nome) values ($1) on conflict (nome) do nothing",
          [arquivo],
        );
